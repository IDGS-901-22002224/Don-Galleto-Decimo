...
def ventas():
    ...
    usuario = Usuario.query.get(1)  # ID fijo en 3
...
def registrar_venta():
    ...
    if request.method == "POST" and form.validate():
        tipo_galleta_id = int(request.form.get("tipo_galleta"))
        cantidad = int(form.cantidad.data)

        # Validaciones y asignación de lotes (igual que tu código original)
        ...
        lotes_disponibles = (
            LoteGalletas.query.filter(LoteGalletas.galleta_id == tipo_galleta_id)
        ...
        if (cantidad + cantidad_reservada) > existencia_total:
            ...
            flash(
                f"Error: No hay suficiente existencia. Disponible: {max(0, disponible)}",
                "danger",
            )
            return render_template(
                "ventas/registrar_ventas.html",
        ...
        for lote in lotes_disponibles:
            ...
            if disponible > 0:
                lotes_con_disponibilidad.append(
                    {"lote": lote, "disponible": disponible}
                )
        ...
        for lote_info in lotes_con_disponibilidad:
            if cantidad_restante <= 0:
                break
            lote = lote_info["lote"]
            cantidad_a_usar = min(cantidad_restante, lote_info["disponible"])
        ...
        # Agregar o actualizar el detalle en sesión
        item_existente = next(
            (
                item
                for item in session["detalle_venta"]
        ...
        for detalle in session["detalle_venta"]:
            for lote_info in detalle["lotes"]:  # Iterar sobre cada lote en el detalle
                ...
                nuevo_detalle = DetalleVentaGalletas(
                    ...
                    lote_id=lote_info[
                        "lote_id"
                    ],  # Acceder al lote_id dentro de lote_info
                ...
                if lote:
                    ...
                    if lote.existencia < 0:
                        db.session.rollback()
                        flash(
                            f"Error: Existencia insuficiente en lote {lote.id_lote}. La venta no se completó.",
                            "danger",
                        )
                        return redirect(url_for("venta.registrar_venta"))
...
def detalles_venta():
    if request.method == "GET":
        ...
        detalles_query = (
            ...
            .join(LoteGalletas, DetalleVentaGalletas.lote_id == LoteGalletas.id_lote)
            ...
            .filter(DetalleVentaGalletas.venta_id == id_venta)
            .all()
        )
...
def obtener_galletas(tipo_venta_id):
    galletas = (
        db.session.query(Galleta).filter(Galleta.tipo_galleta_id == tipo_venta_id).all()
    )

    galletas_json = [
        {"id": g.id_galleta, "nombre": f"{g.galleta} - {g.tipo.nombre}"}
        for g in galletas
    ]
...
def obtener_ticket(venta_id):
    try:
        ...
        if not venta.ticket:
            return (
                jsonify({"error": "No se encontró el ticket en la base de datos"}),
                404,
            )
...
def eliminar_detalle(session_id):
    if "detalle_venta" in session:
        if (
            0 <= session_id < len(session["detalle_venta"])
...
def corte_caja():
    ...
    if request.method == "POST" and form.validate():
        ...
        total_venta = (
            ...
            .filter(Venta.fecha == fecha_corte)
            .scalar()
        )
    ...
    return render_template(
        "ventas/corte_caja.html", form=form, active_page="ventas", cortes=cortes
    )
...
def pedido_portal():
    ...
    return render_template(
        "ventas/pedidos_portal.html", active_page="ventas", pedidos=pedidos
    )
...
def registrar_venta(id_orden, total, tipo_venta, detalles):
    try:
        ...
        )  # Devolver el ID de la venta para poder redirigir si es necesario
...
def cobrar_pedido(id_orden):
    ...
    try:
        ...
        detalles = (
            db.session.query(DetalleVentaOrden, Galleta)
            .join(Galleta, Galleta.id_galleta == DetalleVentaOrden.galletas_id)
        ...
        for d, galleta in detalles:
            ...
            galletas_vendidas[galleta.id_galleta] = (
                galletas_vendidas.get(galleta.id_galleta, 0) + d.cantidad
            )

            # Obtener lotes disponibles ordenados por fecha (más antiguos primero)
            lotes_disponibles = (
                db.session.query(LoteGalletas)
            ...
            for lote in lotes_disponibles:
                ...
                detalle_venta = DetalleVentaGalletas(
                    ...
                    lote_id=lote.id_lote,
                    cantidad=cantidad_a_usar,
                    subtotal=cantidad_a_usar
                    * (d.subtotal / d.cantidad),  # Mantener proporción del subtotal
                )
...
def detalles_pedido():
    if request.method == "GET":
        ...
        detalles_query = (
            ...
            .join(TipoGalleta, Galleta.tipo_galleta_id == TipoGalleta.id_tipo_galleta)
            .filter(DetalleVentaOrden.orden_id == orden_id)
            .all()
        )
...
def merma_galletas():
    ...
    return render_template(
        "ventas/registrar_merma.html", active_page="ventas", form=form
    )
...
def obtener_lotes(galleta_id):
    ...
    for item in session["detalle_venta"]:
        if item.get("id_galleta") == galleta_id:
            for lote in item.get("lotes", []):
                reservas.append(
                    {"lote_id": lote["lote_id"], "cantidad": lote["cantidad"]}
                )
