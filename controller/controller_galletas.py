...
def obtener_galletas_unidad_con_existencia():
    subq = (
        db.session.query(
            LoteGalletas.galleta_id, func.sum(LoteGalletas.existencia).label("total")
        )
        .group_by(LoteGalletas.galleta_id)
        .subquery()
    )

    return (
        db.session.query(Galleta)
...
def galletas():
    ...
    merma_form.galleta_id.choices = [
        (g.id_galleta, g.galleta) for g in galletas_con_existencia
    ]
...
def agregar_galleta():
    ...
    form.receta_id.choices = [
        (r.idReceta, r.nombreReceta) for r in Receta.query.filter_by(estatus=1).all()
    ]
...
def merma_galleta():
    ...
    if form.validate_on_submit():
        ...
        if not lote:
            flash(
                "No hay lotes disponibles con existencia para esta galleta.", "warning"
            )
            return redirect(url_for("galletas.galletas"))

        if cantidad > lote.existencia:
            flash(
                f"La cantidad solicitada ({cantidad}) excede la existencia del lote ({lote.existencia}).",
                "danger",
            )
            return redirect(url_for("galletas.galletas"))
