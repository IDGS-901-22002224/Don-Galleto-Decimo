...
from forms.produccion_forms import (
    LoteGalletasForm,
    MermaGalletaForm,
    MermaInsumoForm,
    EmpaqueForm,
...
def produccion():
    ...
    merma_insumo_form.insumo_id.choices = [
        (i.id_insumo, i.nombreInsumo) for i in Insumos.query.all()
    ]
    form_empaque = EmpaqueForm()
    usuario = Usuario.query.get(1)
    galletas_con_existencia = (
        db.session.execute(
            text(
                """
        SELECT 
    ...
    form_empaque.galleta_id.choices = [
        (g["id_galleta"], f"{g['galleta']} (Total: {g['total']})")
        for g in galletas_con_existencia
    ]

    form.galleta_id.choices = [
        (g.id_galleta, g.galleta)
        for g in Galleta.query.filter_by(tipo_galleta_id=tipo_unidad).all()
    ]

    merma_form.lote_id.choices = [
        (l.id_lote, f"{l.galleta.galleta} | {l.fechaProduccion}")
        for l in LoteGalletas.query.order_by(LoteGalletas.fechaProduccion.desc()).all()
    ]
    ...
    if (
        request.method == "GET"
        and "galleta_id" in request.args
        and "abrir_modal_empaque" not in request.args
    ):
    ...
    if form.validate_on_submit():
        ...
        for ingrediente in ingredientes:
            ...
            cantidad_total = (
                cantidad_base * form.cantidad.data // receta.cantidad_galletas
            )

            insumo = (
                db.session.execute(
                    text(
                        "SELECT * FROM insumos WHERE nombreInsumo = :nombre AND unidad = :unidad"
                    ),
            ...
            if not insumo:
                flash(
                    f"Insumo '{nombre_ingrediente}' con unidad '{unidad}' no encontrado.",
                    "danger",
                )
            ...
            lotes = (
                db.session.execute(
                    text(
                        "SELECT * FROM loteinsumo WHERE id_insumo = :id AND cantidad > 0 ORDER BY fechaIngreso ASC"
                    ),
                    {"id": insumo["id_insumo"]},
                )
                .mappings()
            ...
            for lote in lotes:
                ...
                if lote["cantidad"] >= cantidad_faltante:
                    db.session.execute(
                        text(
                            "UPDATE loteinsumo SET cantidad = cantidad - :descontar WHERE idLote = :id"
                        ),
            ...
            if cantidad_faltante > 0:
                flash(
                    f"No hay suficiente '{nombre_ingrediente}' para producir {form.cantidad.data} galletas.",
                    "danger",
                )
                db.session.rollback()
                return redirect(url_for("produccion.produccion"))
            ...
            db.session.execute(
                text(
                    "UPDATE insumos SET total = total - :cantidad WHERE id_insumo = :id"
                ),
    ...
    if merma_form.validate_on_submit():
        ...
        if lote.existencia < cantidad_merma:
            flash(
                f"La cantidad de merma excede la existencia del lote ({lote.existencia}).",
                "danger",
            )
            return redirect(url_for("produccion.produccion"))
        ...
        flash(
            f"Merma registrada correctamente para '{lote.galleta.galleta}'.", "success"
        )
        return redirect(url_for("produccion.produccion"))

    # ==========================
    # Mostrar lotes y alertas
    # ==========================
    galletas = (
        db.session.execute(
            text(
    ...
    return render_template(
        ...
        lote_merma=lote_merma,
        alertas_agotamiento=alertas_agotamiento,
...
def eliminar_lote():
    ...
    flash(
        f"El lote de la galleta '{galleta_nombre}' ha sido eliminado con éxito.",
        "success",
    )
    return redirect(url_for("produccion.produccion"))
...
def merma_insumo():
    ...
    lotes = (
        LoteInsumo.query.filter(LoteInsumo.id_insumo == insumo_id)
        .filter(LoteInsumo.cantidad > 0)
        .order_by(LoteInsumo.fechaIngreso.asc())
        .all()
    )
...
def empaquetar():
    ...
    if (
        not tipo_empaque
        or not galleta_id
        or not cantidad_empaques
        or not cantidad_por_empaque
    ):
    ...
    lotes = (
        LoteGalletas.query.filter_by(galleta_id=galleta_id)
    ...
    if existencia_total < total_galletas_necesarias:
        flash(
            f"No hay suficientes galletas para empaquetar. Se necesitan {total_galletas_necesarias} y solo hay {existencia_total}.",
            "danger",
        )
        return redirect(url_for("produccion.produccion"))
    ...
    galleta_destino = Galleta.query.filter_by(
        galleta=galleta.galleta, tipo_galleta_id=tipo_id
    ).first()

    if not galleta_destino:
        flash(
            f"No existe un registro para empaquetado tipo '{tipo_nombre}' de la galleta '{galleta.galleta}'.",
            "danger",
        )
        return redirect(url_for("produccion.produccion"))
