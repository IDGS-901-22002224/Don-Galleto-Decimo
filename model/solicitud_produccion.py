...
class SolicitudProduccion(db.Model):
    ...
    detalleorden_id = db.Column(
        db.Integer,
        db.ForeignKey("detalleVentaOrden.id_detalleVentaOrden"),
        nullable=False,
    )
    ...
    detalleOrden = db.relationship(
        "DetalleVentaOrden", backref=db.backref("solicitudesProduccion", lazy=True)
    )
