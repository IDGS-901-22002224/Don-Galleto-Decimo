...
def get_ventas_diarias():
    ...
    result = (
        db.session.query(Venta.fecha, db.func.sum(Venta.total).label("total_ventas"))
...
def get_productos_mas_vendidos():
    ...
    result = (
        db.session.query(
            Galleta.galleta,
            db.func.sum(DetalleVentaGalletas.cantidad).label("cantidad_vendida"),
        )
        .join(LoteGalletas, LoteGalletas.galleta_id == Galleta.id_galleta)
        .join(
            DetalleVentaGalletas, DetalleVentaGalletas.lote_id == LoteGalletas.id_lote
        )
